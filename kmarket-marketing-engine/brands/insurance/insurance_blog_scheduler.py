...
class InsuranceBlogScheduler:
    """InsureBalance 정기 자동 발행 스케줄러 레고 블록"""
    ...
    def _get_engine(self):
        if self._engine is None:
            ...
            self._engine = InsuranceBlogEngine()
        return self._engine
    ...
    def _trigger_publish(self):
        ...
        try:
            ...
            logger.info(f"🎉 [InsuranceScheduler] 정기 자동 발행 성공: {res.get('article_title')}")
        except Exception as e:
            logger.error(f"❌ [InsuranceScheduler] 정기 자동 발행 실패: {e}")

    def run_once_now(self) -> Dict[str, Any]:
        """수동 1회 즉시 실행 (대시보드 테스트용)"""
        logger.info("⚡ [InsuranceScheduler] 수동 1회 즉시 발행 트리거")
        engine = self._get_engine()
