...
class InsuranceTistoryPublisher:
    """InsureBalance 보험 전용 티스토리 블로그 자동 발행 엔진"""
    ...
    async def publish_post_async(
        ...
        if not self.is_available():
            logger.warning("⚠️ [Tistory-Insurance] tistory_session.json 세션 파일이 없습니다.")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(f"🌐 [Tistory-Insurance] 블로그 목록 진입: {write_url}")
                ...
                if await complete_btn.count() > 0:
                    ...
                    if await publish_btn.count() > 0:
                        ...
                        logger.info(f"🎉 [Tistory-Insurance] 발행 완료: {post_url}")
                ...
                logger.error(f"❌ [Tistory-Insurance] 발행 실패: {e}")
                return {"status": "error", "message": str(e), "blog_name": self.blog_name}
            finally:
                await browser.close()
