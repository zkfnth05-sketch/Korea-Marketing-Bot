...
class InsuranceBlogScenarioDirector:
    ...
    PERSONAS = {
        "p1_young_starter": {
            ...
            "tone_description": "월급 아끼려고 고정비 다이어트에 진심인 2030 직장인. 통장에서 매달 빠져나가던 보험료 뜯어보고 깜짝 놀라 7만 원 줄인 찐후기 구어체 (~하더라고요, ~했더니 소름 돋았음ㅋㅋ).",
...
1. 분량 및 모바일 최적화 호흡:
   ...
   - 본문 **맨 처음에 16:9 감성 대표 사진 1장이 삽입**되고 그 밑으로 본문이 시작됨.
   - 모바일 독자의 80%는 정독하지 않고 5~10초 만에 스크롤을 훑어보므로, **1~2줄 단위로 시원하게 줄바꿈**.
   - 눈에 쏙 들어오는 이모지(🚨, 💸, 💡, 🔖, 👉, 🚗, 🧠, 🏥, 👶) 적극 활용.
...
3. 스키머(훑어보는 사람) 시선 장악용 [2단 비주얼 훅 박스] (본문에 필수 삽입):
...
{{
  ...
  "body_markdown": "2단 비주얼 박스와 이미지 앵커, 모바일 1~2줄 호흡이 완벽히 구현된 1,300~1,500자 완성형 본문",
