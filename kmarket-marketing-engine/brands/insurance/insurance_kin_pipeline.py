# -*- coding: utf-8 -*-
"""
Insurance Kin Pipeline (🚀 InsureBalance 전용 네이버 지식iN 실시간 낚아채기 원스톱 오케스트레이터)
========================================================================================
- 브랜드: InsureBalance (보험 다이렉트 비교 & AI 리모델링)
- 역할:
  1. 🔍 100대 황금 키워드 기반 실시간 보험 질문 스캔 (InsuranceKinScanner)
  2. 🧠 AI 적합도 85점 심사 및 1,500자~4,000자 초고밀도 3박자 킬러 답변 생성 (InsuranceKinGeminiSolver)
  3. 🤖 네이버 지식iN 자동 답변 등록 (InsuranceKinPublisher)
  4. 📁 히스토리 DB (data/insurance_kin_history.json) 저장 및 대시보드 연동
"""
...
logger = logging.getLogger("InsuranceKinPipeline")
...
from brands.insurance.insurance_kin_scanner import InsuranceKinScanner
from brands.insurance.insurance_kin_gemini_solver import InsuranceKinGeminiSolver
from brands.insurance.insurance_kin_publisher import InsuranceKinPublisher
from brands.insurance.insurance_kin_python_filter import InsurancePythonFilter


class InsuranceKinPipeline:
    """🛡️ InsureBalance 전용 네이버 지식iN 실시간 낚아채기 파이프라인"""

    BRAND = "insurance"
    NAME = "InsureBalance (보험비교)"

    def __init__(self):
        self.scanner = InsuranceKinScanner()
        self.solver = InsuranceKinGeminiSolver()
        self.publisher = InsuranceKinPublisher()
        self.filter = InsurancePythonFilter()
    ...
    def run_catch_cycle(self, max_catch: int = 10, dry_run: bool = False) -> Dict[str, Any]:
        ...
        2. 적합도 심사 (85점 이상 선별)
        3. 합격한 질문 중 성공 등록될 때까지 순차 시도 (하루 10개 할당량 도달 시 중단)
        """
        ...
        if today_published >= self.DAILY_QUOTA:
            logger.info(f"✅ [Insurance 지식iN] 오늘 이미 {today_published}개 성공 등록 완료 — 일일 할당량({self.DAILY_QUOTA}개) 달성!")
        ...
        for q in scanned_questions:
            ...
            logger.info(f"📊 [Insurance 필터 심사] {score}점 (합격: {is_passed}) | 제목: {q['title'][:30]}... ({reason})")
        ...
        if not passed_candidates:
            ...
            return {"success": False, "status": "no_passed_questions", "message": "적합도 50점 이상 보험 질문 대기 중"}
        ...
        for candidate in passed_candidates:
            ...
            if remaining_quota <= 0:
                logger.info(f"🎯 [Insurance 지식iN] 오늘 목표 {self.DAILY_QUOTA}개 달성! 사이클 종료")
                break
            ...
            if real_content:
                ...
                # 🔍 [핵심 2차 필터 심사]: 실제 수집된 본문 기준으로 보험 적합도 및 앵커 키워드 재검증
            ...
            if dry_run:
                pub_res = {
                    ...
                    "mode": "dry_run",
                    "published_url": f"https://kin.naver.com/qna/detail.naver?d1id=4&dirId=40103&docId={t_question['doc_id']}&dry_run=true",
        ...
        logger.info(f"🏁 [Insurance 지식iN 사이클 완료] 이번 사이클 성공: {published_count}개 | 오늘 누적: {total_today}/{self.DAILY_QUOTA}개")
        return {
...
if __name__ == "__main__":
    ...
    print("Insurance Pipeline Result:", json.dumps(res, ensure_ascii=False, indent=2))
