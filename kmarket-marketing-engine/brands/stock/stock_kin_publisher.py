# -*- coding: utf-8 -*-
"""
Stock Kin Publisher (🤖 Stock Master 전용 네이버 지식iN Playwright 자동 답변 등록기)
========================================================================================
- 브랜드: Stock Master (주식 AI)
- 역할:
  1. brands/stock/naver_browser_profile 및 naver_session.json 쿠키 자동 마운트
...
logger = logging.getLogger("StockKinPublisher")

CURRENT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = CURRENT_DIR.parent.parent


class StockKinPublisher:
    """📈 Stock Master 전용 네이버 지식iN 자동 답변 등록기"""
    ...
    async def async_publish_answer(self, question_url: str, answer_text: str, source_url: str = "") -> Dict[str, Any]:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                # 2. 질문 상세 페이지 접속
                logger.info(f"🌐 [Stock 영구 프로필 지식iN 접속] {question_url}")
                ...
                if "nidlogin.login" in page.url:
                    logger.warning("🚨 Stock Master 네이버 로그인 세션이 필요합니다.")
                    await context.close()
                    return {
                        "success": False,
                        "status": "login_required",
                        "message": "Stock Master 네이버 1회 로그인 세션 저장이 필요합니다",
                        "question_url": question_url
                    }
                ...
                if editor_area:
                    ...
                    try:
                        ...
                        pasted = True
                        logger.info("📋 [무인 스텔스 클립보드 무손실 주입 성공] URL 및 퀀트 카드 서식 100% 보존")
                        ...
                        try:
                            ...
                            await page.wait_for_timeout(1500)
                            pasted = True
                            logger.info("📋 [클립보드 무손실 붙여넣기 성공] URL 및 퀀트 카드 서식 100% 보존 주입")
                    ...
                    logger.info(f"🎉 Stock Master 지식iN 답변 등록 완료! (현재 URL: {final_url})")
                ...
                logger.warning(f"Stock 지식iN 답변 등록 중 예외: {e}")
                await context.close()
                return {
                    "success": False,
                    "status": "error",
...
if __name__ == "__main__":
    publisher = StockKinPublisher()
    sample_url = "https://kin.naver.com/qna/detail.naver?dirId=40102&docId=494614491"
    sample_answer = "삼성전자 주가 및 반도체 밸류에이션 실전 분석 내용..."
    res = publisher.publish_answer(sample_url, sample_answer, "https://stockmaster-ai.vercel.app/")
