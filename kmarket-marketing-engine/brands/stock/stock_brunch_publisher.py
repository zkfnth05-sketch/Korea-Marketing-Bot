...
- 감성적이고 통찰력 있는 투자 칼럼형 제목(title_brunch) 및 본문 자동 작성
"""
...
class StockBrunchPublisher:
    """StockMaster 주식 전용 브런치스토리 자동 발행 엔진"""
    ...
    async def publish_story_async(
        ...
        if not self.is_available():
            logger.warning("⚠️ [Brunch-Stock] brunch_session.json 세션 파일 부재")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.error(f"❌ [Brunch-Stock] 발행 실패: {e}")
                return {"status": "error", "message": str(e)}
            finally:
                await browser.close()
