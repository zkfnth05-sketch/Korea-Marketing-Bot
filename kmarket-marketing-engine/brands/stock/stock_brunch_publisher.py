# -*- coding: utf-8 -*-
"""
📈 StockMaster Brunch Publisher (주식 전용 브런치스토리 무인 자동 발행 레고 블록)
================================================================================
- 크롬 영구 프로필 디렉터리(brunch_chrome_profile) 및 세션(brunch_session.json)을 활용한 100% 무인 자동 발행
- 1. 로컬 안전 백업 보관 (outputs/stock/brunch/brunch_draft_topic_xxx.json)
...
BRUNCH_OUT_DIR = PROJECT_ROOT / "outputs" / "stock" / "brunch"

logger = logging.getLogger("StockBrunchPublisher")


class StockBrunchPublisher:
    """StockMaster 주식 전용 브런치스토리 자동 발행 엔진 (영구 크롬 프로필/세션 지원)"""
    ...
    def publish_story(
        ...
        try:
            ...
            logger.error(f"❌ [Brunch-Stock] 발행 예외: {e}")
    ...
    async def publish_story_async(
        ...
        try:
            ...
            logger.warning(f"⚠️ [Brunch-Stock] 로컬 보관 예외: {e}")

        if not self.is_available():
            logger.warning("⚠️ [Brunch-Stock] 브런치 영구 프로필 또는 세션 파일이 없습니다. 로컬 보관 모드로 유지됩니다.")
            return {
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                logger.info(f"🌐 [Brunch-Stock] 에디터 진입: {write_url}")
                ...
                logger.error(f"❌ [Brunch-Stock] 자동 발행 예외: {e}")
