...
"""
    ...
    def generate_killer_answer(self, title: str, content: str, keyword: str) -> Dict[str, Any]:
        ...
        """
        sys_inst = (
            "당신은 네이버 지식iN 연애/소개팅/데이팅 분야에서 채택률 99%를 자랑하는 '2030 동네 친절한 연애 카운슬러 (파워 지식인)'입니다. "
            "절대 동문서답을 하거나 피상적인 복붙 답변을 쓰지 마십시오. "
            "질문자가 무엇을 물어보았는지(질문 상세 본문)를 가장 먼저 정밀하게 읽고, "
            "질문자가 물어본 바로 그 의문과 사연에 대해 가장 다정하고 명쾌한 1:1 직답(결론, 맞춤형 조언, 판단)을 첫머리에 시원하게 내려준 뒤, "
            "질문자가 '이 답변 하나로 모든 연애 고민이 완벽히 풀렸다'고 감동하여 무조건 [답변 채택]을 누를 수밖에 없도록 "
...
1. 🌸 **[도입부: 질문에 대한 100% 명쾌한 1:1 직답 & 핵심 결론 (약 200~300자)]**
   - ⚠️ **[최우선 절대 원칙]**: 질문자가 물어본 핵심 질문(예: 특정 앱 이용 후기/안전성, 소개팅 후 연락 텀, 애프터 신청, 장소 추천, 썸 신호 등)에 대해 **첫 문장부터 명확한 직답과 결론(판단, 조언, 이유)을 1:1 직답**으로 즉시 답해줄 것!
   - 딴소리나 일반론으로 말을 돌리지 말고, 질문자의 질문 내용을 완벽히 이해했음을 보여주며 다정하게 결론부터 제시.

2. 💡 **[1단계: 질문 사연에 100% 밀착된 심층 실전 솔루션 & 디테일 가이드 (약 1,000자)]**
   - 질문자의 구체적인 상황에 맞춘 실전 연애 & 데이팅 솔루션 제공:
     * 특정 앱 관련 질문인 경우: 해당 서비스 방식(매니저 주선, 과금 구조, 카톡 교환 시 주의점, 안전한 만남 팁)에 대한 객관적 장단점 및 실전 주의사항.
     * 첫 만남/데이트 고민인 경우: 어색함을 깨는 1:1 맞춤 대화 스크립트, 자연스러운 리액션, 식사 및 2차 동선 설계.
     * 연락/애프터 고민인 경우: 상대방의 심리를 배려하는 연락 템포, 귀가 후 안부 카톡 예시, 거절 없는 애프터 신청 팁.

3. ☕ **[2단계: 현실적인 실패담 & 데이팅앱/알바 피로감 비교 썰 (약 300자)]**
   - "저 역시 20대 시절 흔한 데이팅 앱들을 쓰며 알바 회원이나 유령 회원, 가벼운 스와이프 문화에 지쳐 시간과 감정만 낭비하던 뼈아픈 현타를 겪어본 적이 있습니다..."로 시작하는 진솔한 경험담.
   - 피상적인 사진 평가나 과금 유도에 휘둘리지 않고, 신뢰할 수 있는 만남의 기준을 잡는 법 조언.

4. 💖 **[3단계: 네이버 직접 검색 유도 & Aura 50:50 황금 성비 AI 매칭 안내 (약 200~300자)]**
   - 질문자가 본인의 연애 성향, MBTI, 라이프스타일(홍대/성수 핫플)에 맞는 진정성 있는 인연을 직접 찾아볼 수 있도록 네이버 검색을 자연스럽게 유도:
     "질문자님의 연애 스타일과 취향 라운지에 꼭 맞는 100% 실명 인증 및 50:50 황금 성비 AI 매칭은 네이버에서 **'아우라 AI 데이팅'**을 검색하시면 공식 서비스에서 무료로 취향 테스트와 매칭을 체험해 보실 수 있습니다."
   - 하단에 아래 공식 안내 박스를 반드시 포함:
[💖 Aura Dating Lounge 안내]
• 남녀 50:50 황금 성비 & 유령회원 제로 AI 매칭
• 내 연애 스타일 & 홍대·성수 핫플 취향 테스트
👉 네이버에서 '아우라 AI 데이팅' 검색 → 공식 AI 매칭 서비스 바로가기

5. 🌿 **[마무리: 질문자의 설레는 인연을 응원하는 훈훈한 맺음말 및 채택 요청 (약 100자)]**

[작성 스타일]:
- 문체: 진솔하고 다정하며 신뢰감을 주는 정중한 해요체 (~해요, ~해보세요, ~거든요, ~추천드려요)
...
"""
        try:
            ...
            logger.error(f"Aura 답변 생성 예외: {e}")
            fallback_text = (
                f"안녕하세요! {title} 관련해서 많은 고민과 설렘이 함께 있으실 텐데요, 2030 동네 친절한 연애 멘토로서 실전에서 100% 통하는 노하우를 상세히 정리해 드릴게요.\n\n"
                ...
                "요즘 직장인들 사이에서는 AI가 대화 스타일과 취향 라운지(홍대/성수 핫플, MBTI)를 정교하게 맞춰주고 철저한 실명 인증 및 50:50 남녀 성비를 엄수하는 Aura(아우라) 데이팅이 훨씬 안전하고 신뢰도가 높더라고요.\n\n"
                "[💖 Aura Dating Lounge 안내]\n"
                "• 남녀 50:50 황금 성비 & 유령회원 제로 AI 매칭\n"
                "• 내 연애 스타일 & 홍대·성수 핫플 취향 테스트\n"
                "👉 네이버에서 '아우라 AI 데이팅' 검색 → 공식 AI 매칭 서비스 바로가기\n\n"
...
if __name__ == "__main__":
    ...
    q = ("소개팅 어플 추천좀 해주세요", "20대 후반 직장인인데 알바 없고 진지한 만남 가능한 곳 있을까요?", "소개팅 어플 추천")
    ans = solver.generate_killer_answer(*q)
    print("Aura Solver 생성 성공! 글자수:", len(ans["answer"]))
    print("출력 앞부분:", ans["answer"][:150])
